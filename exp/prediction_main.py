...
def main():
    ...
    if is_distributed:
        ...
        mp.spawn(main_worker, nprocs=local_size, args=(cfgs, node_rank, local_size, cfgs['world_size'], cfg_file_path), join=True)
...
def main_worker(local_rank, cfgs, node_rank, local_size, world_size, cfg_file_path, param_search_res_df_path=None, record_param_name=None, base_log_dir = '../log'):
    ...
    if is_distributed:
        ...
        dist.init_process_group('nccl', init_method=f"tcp://{os.environ['MASTER_ADDR']}:{os.environ['MASTER_PORT']}",
    ...
    m = model_h_vector_embed_mean.TaskModel(base_model=cfgs['model_name'], task_type='prediction',
                                            input_size=input_size, output_size=input_size,
                                            coupled_len=cfgs['coupled_len'], pred_len=cfgs['pred_len'], seq_tcn=True,
                                            inverse_out=cfgs['inverse_out'], data_normalizer=train_dataset.normalizer,
                                            **cfgs['model_params'])
...
def train(train_loader, model, optimizer, epoch, epoches, device, loss_weights):
    model.train()
    ...
    def run_validate(val_loader, base_progress=0):
        with torch.no_grad():
            ...
            for i, batch_data in enumerate(val_loader):
                ...
                preds.append(out_dict['pred_outs'][:, -cfgs['pred_len']:])
